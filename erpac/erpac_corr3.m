function [pacA, pacB, pacC, zdiff, pdiff] = erpac_corr3(data, srate, eventsA, eventsB, eventsC, timeWindow, lo_freq_phase, hi_freq_phase, lo_freq_amp, hi_freq_amp)
...
[foo, phaseC] = ERP(phasedata, srate, eventsC, timeWindow(1), timeWindow(2), 1); clear foo;
[foo, ampC] = ERP(ampdata, srate, eventsC, timeWindow(1), timeWindow(2), 1); clear foo;

% initialize
pacA = zeros([1 size(phaseA, 2)]);
pacB = zeros([1 size(phaseA, 2)]);
pacC = zeros([1 size(phaseA, 2)]);
...
for t = 1:size(phaseA, 2)
    ...
    pacC(t) = circ_corrcl(phaseC(:, t), ampC(:, t));
