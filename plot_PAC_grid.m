...
for elec = elecs
    ...
    subplot(16,16,elec);
    for iCond = 1:3
        inds = setdiff(PAC_cond{elec_locs(elec)}.cond==iCond,   PAC_cond{elec_locs(elec)}.bad_trials);
        dat = mean(abs(mean(PAC_cond{elec_locs(elec)}.power(inds,round((bl+st)*srate):round((bl+en)*srate)),1)));
